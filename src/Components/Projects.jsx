...
export default function Projects() {
  ...
  const projects = ProjectsData.map((project) => {
    ...
    const techStack = project.techStack.map((technology) => {
      ...
      return (
        <span style={style} className="project--technology">
          {technology}
        </span>
      );
    });
    ...
    const collaborators = project.collaborators.map((collaborator, index) =>
      index === collaboratorSize - 1 ? collaborator : collaborator + ", "
    );
    ...
    const gameDiv = (
      <>
        {isGameRunning ? (
          ...
          >
            <button
              className="project--source button-30"
    ...
    return (
      <div className="project--component" id="PROJECTS">
        <div className="project--information">
          <h3 style={{ paddingLeft: "10px", fontSize: "24px" }}>
            <u>{project.projectName}</u>
          </h3>

          <h3 style={{ marginLeft: "12px" }}>Tech Stack:</h3>
          <div style={{ display: "flex", flexWrap: "wrap" }}>{techStack}</div>
          ...
          {project.note !== "" && (
            <em className="project--note" style={{ marginLeft: "10px" }}>
              Note: {project.note}{" "}
            </em>
          )}
