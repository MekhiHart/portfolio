...
export default function Contact(props) {
  ...
  const invalidMessage = (
    <div id="contact--invalidMessage">Please fill out this field.</div>
  );
  ...
  const formComponent = (
    <>
      ...
      <form
        ...
        onSubmit={(event) =>
          submitForm(event, setIsInvalidSubject, formState, setFormState)
        }
      >
        <select
          id="contact--subject"
          ...
          name="subject"
          value={subject}
          onChange={(event) => handleFormChange(event, setIsInvalidSubject)}
        >
        ...
        <input
          type="submit"
          id="contact--submit"
          className="button-30 project--source"
  ...
  const successfulDiv = (
    <>
      <img
        className="shark--asset afterSendingForm"
        src={starShark}
        style={{ width: "140px", height: "160px" }}
      />
      <h2 className="form--state" style={{ color: "white" }}>
        Message was successfully sent to Mekhi!
      </h2>
    </>
  );

  const unsuccsessfulDiv = <div>Email Not Sent</div>;
